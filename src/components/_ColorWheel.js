// Color Wheel with D3
// http://github.com/4s7r0/Vagor
// J. Adam Moore
// MIT License

  const ColorWheelMarkerDatum = (color, name, show) => {
    colorState = tinycolor(color).toHsv();
    nameState = name;
    showState = show;
  };
  ...
  ColorWheel.prototype.updateHarmony = function (target, theta) {
    ...
    switch (this.currentMode) {
      ...
      case ColorWheel.modes.MONOCHROMATIC:
      case ColorWheel.modes.SHADES:
      case ColorWheel.modes.TRIADIC:
      case ColorWheel.modes.ASYMETRICAL_COMPLEMENTARY:
      case ColorWheel.modes.TETRADIC:
      case ColorWheel.modes.DOUBLE_COMPLEMENTARY:
